...
-- -------------------------------------------------------
-- student_id: unique per student, derived from email local-part
-- -------------------------------------------------------
...
declare
  ...
  suffix int := 2;
begin
  base := lower(trim(split_part(p.email, '@', 1)));
  if base = '' then
    base := 'user';
  end if;
...
-- Admins delete user accounts (auth.users cascade removes profile and related rows)
